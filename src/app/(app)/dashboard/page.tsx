...
function FreqCard({ freq, isSelected, isRecommended, onClick, lang, locked }: { freq: Frequency; isSelected: boolean; isRecommended: boolean; onClick: () => void; lang: Lang; locked?: boolean }) {
  return (<button onClick={onClick} style={{background:isSelected?`linear-gradient(145deg,${freq.glowColor},rgba(8,16,32,0.8))`:'rgba(8,16,32,0.6)',border:`1px solid ${isSelected?freq.color+'50':'rgba(61,142,207,0.06)'}`,borderRadius:'16px',padding:'20px',cursor:'pointer',textAlign:'left',position:'relative',transition:'all 0.4s cubic-bezier(0.4,0,0.2,1)',overflow:'hidden',transform:isSelected?'scale(1.02)':'scale(1)',opacity:locked?0.45:1}}>
    ...
    {locked&&<div style={{position:'absolute',top:'10px',right:'10px',background:'rgba(201,168,76,0.15)',color:'#c9a84c',fontSize:'9px',fontWeight:800,padding:'4px 10px',borderRadius:'6px',letterSpacing:'1px',textTransform:'uppercase',display:'flex',alignItems:'center',gap:'4px',border:'1px solid rgba(201,168,76,0.3)'}}>🔒 PRO</div>}
    ...
    <>
      ...
      <div style={{background:'radial-gradient(ellipse at 30% 20%,#0f2035,#081020 50%,#050c18)',minHeight:'100vh',padding:'32px 24px',fontFamily:"'Outfit',sans-serif",position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1,maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'12px',marginBottom:'36px'}}>{FREQUENCIES.map(f=>{const freeAllowed=[528,396];const isLocked=userPlan==='free'&&!freeAllowed.includes(f.hz);return <FreqCard key={f.hz} freq={f} isSelected={selectedFrequency?.hz===f.hz} isRecommended={recommendedHz===f.hz} locked={isLocked} onClick={()=>{if(isLocked){setUpgradeModalReason('frequency');setUpgradeModalOpen(true);return;}setSelectedFrequency(f);}} lang={lang}/>;})}</div>
          <div style={{marginBottom:'36px'}}><p style={{fontSize:'10px',color:'rgba(255,255,255,0.25)',textTransform:'uppercase',letterSpacing:'2px',marginBottom:'14px'}}>{t(lang,'Ambient layers','Capas ambientales')} <span style={{color:'rgba(255,255,255,0.15)',fontStyle:'italic',textTransform:'none'}}>{t(lang,'optional','opcional')}</span></p><div style={{display:'flex',gap:'10px',flexWrap:'wrap'}}>{AMBIENT_SOUNDS.map(s=>{const a=selectedAmbient===s.id;const isLocked=userPlan==='free'&&s.id!=='none';return <button key={s.id} onClick={()=>{if(isLocked){setUpgradeModalReason('ambient');setUpgradeModalOpen(true);return;}toggleAmbient(s.id);}} style={{background:a?'rgba(61,142,207,0.1)':'rgba(12,26,46,0.5)',border:`1px solid ${a?'rgba(61,142,207,0.25)':isLocked?'rgba(201,168,76,0.2)':'rgba(61,142,207,0.08)'}`,borderRadius:'12px',padding:'10px 20px',color:a?'#4a9eff':isLocked?'rgba(255,255,255,0.3)':'rgba(255,255,255,0.4)',fontSize:'14px',cursor:'pointer',display:'flex',alignItems:'center',gap:'8px',opacity:isLocked?0.6:1}}>{isLocked?<span style={{fontSize:'12px'}}>🔒</span>:<span style={{fontSize:'16px'}}>{s.icon}</span>}{t(lang,s.nameEn,s.nameEs)}</button>;})}</div></div>
          ...
          {!hasRecording&&!isRecording&&<div style={{textAlign:'center',marginBottom:'32px',padding:'14px 24px',background:'rgba(201,168,76,0.03)',border:'1px solid rgba(201,168,76,0.08)',borderRadius:'12px'}}><p style={{fontSize:'13px',color:'rgba(255,255,255,0.4)',margin:0,lineHeight:1.6}}>{t(lang,'✦ Speak clearly for at least 15 seconds. Try reading: "I am choosing to believe in myself. I am calm, I am strong, and I am ready to grow."','✦ Habla con claridad al menos 15 segundos. Prueba leer: "Hoy elijo creer en mí. Estoy en calma, soy fuerte y estoy lista para crecer."')}</p></div>}
          </>)}
          ...
          {generatedAudio&&(<div style={{marginBottom:'24px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',background:'rgba(201,168,76,0.05)',border:'1px solid rgba(201,168,76,0.12)',borderRadius:'10px',padding:'10px 14px',marginBottom:'12px'}}>
            ...
            <audio ref={genAudioRef} src={`data:audio/mp3;base64,${generatedAudio}`} preload="metadata" onLoadedMetadata={(e) => setGenDuration(e.currentTarget.duration)} onTimeUpdate={(e) => setGenProgress(e.currentTarget.currentTime)} onEnded={() => { setGenPlaying(false); setGenProgress(0); }} />
          ...
          <div style={{display:'flex',justifyContent:'center',gap:'12px',flexWrap:'wrap'}}>
            ...
            <button onClick={()=>{if(userPlan==='free'){setUpgradeModalReason('download');setUpgradeModalOpen(true);return;}const a=document.createElement('a');a.href=`data:audio/mp3;base64,${generatedAudio}`;a.download=`afirmia-${selectedFrequency?.hz||528}hz-${selectedDuration}min.mp3`;a.click();}} style={{background:'transparent',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'12px',padding:'16px 32px',fontSize:'14px',fontWeight:500,color:'rgba(255,255,255,0.5)',cursor:'pointer',fontFamily:"'Outfit',sans-serif"}}>{t(lang,'Download','Descargar')}</button>
  ...
  <div onClick={()=>setUpgradeModalOpen(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}}>
    <div onClick={e=>e.stopPropagation()} style={{background:'#0e1424',border:'1px solid rgba(201,168,76,0.2)',borderRadius:20,padding:40,maxWidth:440,width:'100%',textAlign:'center'}}>
      <div style={{fontSize:11,color:'rgba(201,168,76,0.7)',letterSpacing:2,textTransform:'uppercase',fontWeight:700,marginBottom:16}}>
        {upgradeModalReason==='limit'?t(lang,'Limit reached','Limite alcanzado'):upgradeModalReason==='duration'?t(lang,'Pro feature','Funcion Pro'):upgradeModalReason==='frequency'?t(lang,'Pro frequency','Frecuencia Pro'):upgradeModalReason==='ambient'?t(lang,'Pro sounds','Sonidos Pro'):upgradeModalReason==='download'?t(lang,'Pro feature','Funcion Pro'):t(lang,'Upgrade','Upgrade')}
      </div>
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'#fff',fontWeight:400,marginBottom:12}}>
        {upgradeModalReason==='limit'?t(lang,'You used your free audios','Usaste tus audios gratis'):upgradeModalReason==='frequency'?t(lang,'Unlock all frequencies','Desbloquea todas las frecuencias'):upgradeModalReason==='ambient'?t(lang,'Unlock ambient sounds','Desbloquea sonidos ambientales'):upgradeModalReason==='download'?t(lang,'Download & keep your audios','Descarga y guarda tus audios'):t(lang,'Extended tracks on Pro','Audios extendidos en Pro')}
      </h2>
      <p style={{color:'rgba(255,255,255,0.6)',fontSize:14,marginBottom:24,lineHeight:1.6}}>
        {upgradeModalReason==='limit'?t(lang,'Free plan: 3 free audios (one-time). Upgrade to Pro for 50 tracks/month and durations up to 30 minutes.','Plan Free: 3 audios gratis (una vez). Upgrade a Pro para 50 audios al mes y duraciones de hasta 30 minutos.'):upgradeModalReason==='frequency'?t(lang,'Free plan: only 528Hz and 396Hz. Upgrade to Pro for all 8 Solfeggio frequencies.','Plan Free: solo 528Hz y 396Hz. Upgrade a Pro para las 8 frecuencias Solfeggio.'):upgradeModalReason==='ambient'?t(lang,'Free plan: no ambient sounds. Upgrade to Pro for 5 relaxing soundscapes (rain, ocean, stream, birds, forest).','Plan Free: sin sonidos ambientales. Upgrade a Pro para 5 sonidos relajantes (lluvia, oceano, arroyo, aves, bosque).'):upgradeModalReason==='download'?t(lang,'Free plan: listen in the app. Upgrade to Pro to download and keep your audios forever.','Plan Free: solo escuchas en la app. Pasa a Pro para descargar y guardar tus audios para siempre.'):t(lang,'Free plan: 5 minutes max. Upgrade to Pro for tracks up to 30 minutes.','Plan Free: 5 minutos maximo. Upgrade a Pro para audios de hasta 30 minutos.')}
      </p>
